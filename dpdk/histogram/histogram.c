...
/* Create histogram of data of simple IPV4 packets using X msgs
 * carrying DATA_NR integers randomly generated between [0, HISTOG_NR] (of type T).
 * Count how many of them and create histogram.
 * Each lcore writes atomically on the final histog.
 */

/* Parameters for histogram */
#define HISTOG_NR 1024
T_rte final_histog[1024];    								/* Final histog */

/* Global variables (among cores)*/
uint16_t core_que_nr; 									/* Core number */
const char F_WRITE[] = "histogram_ben_proc.txt";
FILE *wfptr;
rte_rwlock_t *rwl;
...
static uint16_t rx_latency(uint16_t port __rte_unused, uint16_t qidx __rte_unused,
	...
	per_lcore_cycles = now;

	return nb_pkts;
}

/* Calculate latency after finishing processing to measure time of sending on Burst */
static uint16_t calc_latency(uint16_t port, uint16_t nb_pkts){
	uint64_t latency = 0;
	...
	latency = (now - per_lcore_cycles);
	/* Processing */
	latency = latency*((double)NS_PER_S/((double)hz_lcore));
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool){
	...
	/* Add RX */
	for(int i = 0; i < core_que_nr; i++){
		rte_eth_add_rx_callback(port, i, rx_latency, NULL);
	}
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			for(int i = 0; i < nb_rx; i++){
				...
				for (int i = 0; i < payload_size/DATA_SIZE_B; i++){
					key = payload[i];
					if(payload[i] > HISTOG_NR || payload[i] < 0)
						rte_exit(EXIT_FAILURE, "Got a wrong key for histog\n");
					
					/* Set data histog */
					rte_atomic32_add(&final_histog[key], 1);
			...
			/* Update tot number of pkts */
			rte_atomic32_add(pkt_rec, nb_mng_rx);
			//printf("Number received pkt is %d\n", rte_atomic32_read(pkt_rec));
			/* Calculate latency */
			if(nb_mng_rx != 0)
				calc_latency(port, nb_mng_rx);
...
int main(int argc, char *argv[]){
	...
	RTE_ETH_FOREACH_DEV(portid) printf("The port id is %d\n", portid);
	/* Check there is only one port */
	nb_ports = rte_eth_dev_count_avail();
	//if (nb_ports != 1){
	//	rte_exit(EXIT_FAILURE, "Need only one port to histog\n");
	//}
	/* Get number of core for optimal usage of queue (1 per core) */
	 ...
	 */
	mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL_HISTOG", NB_MBUF(nb_ports, core_que_nr),
	...
	/* Init with all 0s the final_histog */
    for(int i = 0; i < HISTOG_NR; i++){
            rte_atomic32_init(&final_histog[i]);
        rte_atomic32_set(&final_histog[i], 0);
    }
