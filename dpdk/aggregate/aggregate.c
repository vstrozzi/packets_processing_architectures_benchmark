...
/* Aggregate data of simple IPV4 packet using 1 Mib and 32-bit integers (T) */

/* Parameters for aggregate */

/* Global variables */
uint16_t  core_que_nr;						 			/* Core number */
const char F_WRITE[] = "aggregate_ben_proc.txt";
FILE *wfptr;
rte_rwlock_t *rwl;
...
static int lcore_main(void **args){
	...
	T_rte *aggregate_val = (T_rte *) args[1];
	...
	while(!force_quit){
		...
		//		continue;
			...
			/* Start aggregate on every packet */
			for(int i = 0; i < nb_rx; i++){
				...
				/* Sum up all int32 of the payload */
				T res = 0;
				for (int i = 0; i < payload_size/DATA_SIZE_B; i++)
					res += payload[i];
				/* Write the packet in memory per core */
				rte_atomic32_add(aggregate_val, res);
...
int main(int argc, char *argv[]){
	...
	//if (nb_ports != 1){
	//	rte_exit(EXIT_FAILURE, "Need only one port to aggregate\n");
	//}
	 ...
	 */
	mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL_AGGREG",  NB_MBUF(nb_ports, core_que_nr),
	...
	T_rte *aggregate_val = rte_malloc(NULL, sizeof(T_rte), 0);
	rte_atomic32_init(aggregate_val);
	rte_atomic32_set(aggregate_val, 0);	
