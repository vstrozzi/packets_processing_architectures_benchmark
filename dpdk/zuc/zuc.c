 ...
 *  No need to specify encrypt/decrypt since we are encrypting with a stream cypher
 * /
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool){
	...
	retval = rte_eth_promiscuous_disable(port);
	...
	while(!force_quit){
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			for(int i = 0; i < nb_rx; i++){
				...
				u32 *IV = &payload[0];
				u32 LENGTH = payload_size - sizeof(u32);
				u32 *M = rte_malloc(NULL, LENGTH, 0);
				rte_memcpy(M, payload + 1, LENGTH);
				u32 *C = rte_malloc(NULL, LENGTH, 0);
				/* Encrypt and Decrypt */
				EEA3((u8 *) IV, LENGTH, M, C);
				/* Write resulting-data to packet */
				memcpy(payload + 1, C + 1, LENGTH);
