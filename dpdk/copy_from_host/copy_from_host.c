...
/* Copy to host of data of a simple IPV4 packets
 * using TOT_SIZE_B size, of type T of size DATA_TYPE_B
 */
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			for(int i = 0; i < nb_rx; i++){
				...
				for (int i = 0; i < payload_size/DATA_SIZE_B; i++){
					payload[i] =  rte_atomic32_read(copy_val[i]);
				}
...
int main(int argc, char *argv[]){
	...
	//if (nb_ports != 1){
	//	rte_exit(EXIT_FAILURE, "Need only one port to copy-from-host\n");
	//}
	 ...
	 */
	mbuf_pool = rte_pktmbuf_pool_create("MBUF_POOL_COPY", NB_MBUF(nb_ports, core_que_nr),
