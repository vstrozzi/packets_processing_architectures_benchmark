...
{ 
    ...
    u = L1((W1 << 16) | (W2 >> 16));
    v = L2((W2 << 16) | (W1 >> 16));
    F_R1 = MAKEuint32_t(S0[u >> 24], S1[(u >> 16) & 0xFF],
    S0[(u >> 8) & 0xFF], S1[u & 0xFF]);
    F_R2 = MAKEuint32_t(S0[v >> 24], S1[(v >> 16) & 0xFF],
...
/* Slightly changed since on PsPIN (pass task to be able to allocate memory) */
void EEA3(uint8_t* CK, uint32_t LENGTH, uint32_t* M,uint32_t* C, handler_args_t *args)
{
    /* Fix radio bearer */
    uint32_t BEARER = 120;	
    /* Fix a counter value */
	uint32_t COUNT = 150;
    /* Assuming from RNC to UE */
    ...
    /* Here we assume a maximum length of packet payload of 1500 B */   
    uint32_t *z = (uint32_t *) (((uint8_t*) (args->task->scratchpad[args->cluster_id])) + 2*1500*args->hpu_id + 1500);
