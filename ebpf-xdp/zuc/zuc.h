...
{ 
    u32 *f, v;
    f = bpf_map_lookup_elem(&LFSR_S, 0);
    if(f == NULL){
        exit(0);
    }
    v = MulByPow2(*f, 8);
...
/* Slightly changed since no Malloc on eBPF/XDP */
void EEA3(u8* CK,u32 LENGTH,u32* M,u32* C)
{
    // Fixed arguments
    u32 BEARER = 120;	
	u32 COUNT = 150;
    ...
    // Slight modification to handle reduced stack size
    u32 z = 0;
    ZUC(CK,IV,&z,sizeof(z));
    for (i=0; i<L; i++)
    {
        C[i] = M[i] ^ z;
    }
