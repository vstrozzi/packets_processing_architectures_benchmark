...
/* aggregate data of simple IPV4 packets (port 7999 on ip 10.0.3.0) using 1 Mib and 32-bit integers */


// Define BTF-map array
struct {
...
SEC("aggregate_sec")
int aggregate(struct xdp_md *ctx){
	// Get time of pkt_receival (from boot time and including time of system suspended)
	long long time1 = bpf_ktime_get_ns();
	long long time2 = bpf_ktime_get_ns();
	// Get pkt
		...
		if((void *)ip + sizeof(*ip) <= pkt_end){
			...
			if(ip->protocol == IPPROTO_UDP){
				...
				if((void *)udp + sizeof(*udp) <= pkt_end){
					...
					// Write data to the map on our core position
					__u32 key = 0;
					int32_t *map_val = bpf_map_lookup_elem(&rxaggr, &key);
					// Check not 0 pointer
					if(map_val){
						// Write on slot
						__sync_fetch_and_add(map_val, res);
					}
					
					// Send notification if we finished aggregate
					key = 0;
					...
					// Get time of pkt_processing (from boot time and including time of system suspended) otherwise and send to user space
					if ((void *) payload + 2*sizeof(long long)> pkt_end){
						return XDP_PASS;
					}

					// Write in packet the time of receival and time after processing
					memcpy(payload, &time1, sizeof(long long));
					memcpy(((long long *) payload) + 1, &time2, sizeof(long long));
					// Clear and recompute checksum
					udp->check = 0;
					udp->check = caludpcsum(ip, udp, pkt_end);

					
					// Count number of pkts and 
