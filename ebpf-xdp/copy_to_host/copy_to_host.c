...
/* Copy to host of data of a simple IPV4 packets (port 7999 on ip 10.0.3.0)
   using DATA_TO_COPY_B size of type DATA_TYPE of size DATA_TYPE_B and */
...
SEC("copy_to_host_sec")
int copy_to_host(struct xdp_md *ctx){
			...
			if(ip->protocol == IPPROTO_UDP){
				...
				if((void *)udp + sizeof(*udp) <= pkt_end){
					...
					for (int i = 0; i < payload_size/DATA_SIZE_B; i++){
                        key = i;
							bpf_map_update_elem(&rxcptoh, &key, &payload[i], BPF_ANY);
