...
static void poll_stats(int map_fd){
    ...
    const char fname[] = "copy_to_host_ben.txt";
	...
	while(1){
        ...
        // Packet received, print resulting map
        assert(bpf_map_lookup_elem(map_fd, &key, value) == 0);
        for (int i = 0; i < DATA_NR; i++){
            key = i;
            assert(bpf_map_lookup_elem(map_fd, &key, value) == 0);
            //printf("Got in position %d of memory with stride %lu B, the value %d\n", i, DATA_SIZE_B, *value);
        }
...
int main(int argc, char *argv[])
{
    ...
    prog = xdp_program__open_file("copy_to_host.o", "copy_to_host_sec", NULL);
    if (!prog) {
