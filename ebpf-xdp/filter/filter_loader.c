...
const char map_mem[] = "rxfilter";
const char map_hash[] = "hmfilter";
int pid;
...
void fill_htable(int hash_fd, uint32_t length){
    uint32_t key;
    for (uint32_t i = 0; i < length; i++){
        key = hashmult((uint16_t) i);
        bpf_map_update_elem(hash_fd, &key, &i, BPF_ANY);
    }
}
 ...
 */
static void poll_stats(int map_mem_fd, int map_hash_fd){
    ...
    // Populate hash table
    fill_htable(map_hash_fd, HASH_ENTRIES);
    // Append benchmark to file stats
    const char fname[] = "filter_ben.txt";
...
int main(int argc, char *argv[])
{
    ...
    prog = xdp_program__open_file("filter.o", "filter_sec", NULL);
    if (!prog) {
    ...
    map_mem_fd = bpf_object__find_map_fd_by_name(bpf_obj, map_mem);
    map_hash_fd = bpf_object__find_map_fd_by_name(bpf_obj, map_hash);

    if (map_mem_fd < 0 || map_hash_fd < 0) {
    ...
    printf("Loading!\n");
    // Poll stats every
    poll_stats(map_mem_fd, map_hash_fd);
