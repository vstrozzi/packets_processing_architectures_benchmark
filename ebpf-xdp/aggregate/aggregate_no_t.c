...
/* aggregate data of simple IPV4 packets*/
			...
			if(ip->protocol == IPPROTO_UDP){
				...
				if((void *)udp + sizeof(*udp) <= pkt_end){
					...
					if(nr_pkt){
						...
						const char pkt[] = "We got so far pkt %d\n";
						bpf_trace_printk(pkt, sizeof(pkt), *nr_pkt% (MiB/TOT_SIZE_B));
					...
					// Drop packet since we worked on
					return XDP_DROP;
				}
			}
