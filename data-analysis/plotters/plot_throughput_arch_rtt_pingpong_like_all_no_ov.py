...
labels = ['pingpong', 'kvstore']
fname = ['pingpong', 'kvstore']
    ...
    for set in sets:
        ...
        for lab in fname:
            ...
            for size in SIZES:
                ...
                data_loc.append(round((8*size*chunks_nr)/(time), 4))
...
ax[0, 1].set_title('eBPF-XDP Host (no overhead clock_gettime())')
...
ax[0, 2].set_title('DPDK Host (no overhead clock_gettime())')
ax[0, 2].set_xticks(x, labels)
ax[0, 2].set_ylim([0, 0.21])
ax[0, 2].legend()
...
ax[1, 1].set_title('eBPF-XDP BlueField2 (no overhead clock_gettime())')
...
ax[1, 2].set_title('DPDK BlueField2 (no overhead clock_gettime())')
ax[1, 2].set_xticks(x, labels)
ax[1, 2].set_ylim([0, 0.21])
ax[1, 2].legend()
