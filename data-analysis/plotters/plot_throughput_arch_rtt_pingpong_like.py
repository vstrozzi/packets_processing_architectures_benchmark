...
for arch in archs:  
    for case in cases:
        ...
        for lab in fname:
            data_loc = []
            file = open("dpdk/"  + arch + "_" + lab + "_ben_rtt.txt", "r")
            # Read benchmark for given size
            if lab.startswith("pingpong") or lab.startswith("kvstore"):
                j = 0
                for size in SIZES:
                    chunks_nr = 102400
                    time = 0
...
print(thr_bl2_no_ov)
print("Throughput ration bl2: " + str((1 - thr_host_no_ov/thr_bl2_no_ov)*100))
