...
fname = [f for k in fname for f in k.iterdir() if
        ...
        "overhead" not in str(f.name) and
        "pingpong" in str(f.name)]
...
for lab in fname:
    ...
    file = open(str(lab), "r")
    print(str(lab))
    if lab.name != "host_pingpong_ben_proc.txt" :
        continue
    ...
    for size in SIZES:
        ...
        for i in range(0, chunks_nr):
            ...
            data_loc.append(int(file.readline()))
    ...
    fig = plt.figure()
    for i in range(0, 3):
        t = np.arange(0, chunks_nr, 1)
        ax = plt.gca()
        print(len(t))
        print(pd_dat.iloc[:, i])
        plt.plot(pd_dat.iloc[:, i], label = str(SIZES[i]) + "B", color = colors[i], linewidth=0.01)
        break
    ...
    # Title and labels
    plt.xlabel("Samples", size = 11)
    plt.ylabel('Delay (ns)', size= 11)
    plt.ylim([0, 2000])
    ...
    plt.savefig('plot/pingpong_over_time/' + NAMES[val] + "_" + str(lab.name).replace(".txt", "") + '.png', dpi=500)
    num += 1
